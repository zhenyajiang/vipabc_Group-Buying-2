var COMMON = {
    ...
    showToast2: function (msg, duration) {
        duration = isNaN(duration) ? 2000 : duration;
        var m = document.createElement('div');
        m.innerHTML = msg;
        m.setAttribute('class', 'toastDiv2');
        document.body.appendChild(m);
        setTimeout(function () {
            var d = 0.5;
            m.style.webkitTransition = '-webkit-transform ' + d
    ...
    checkMail2: function (id) {
        ...
        if (!acct) {
            this.showToast2("邮箱不能为空");
            return false;
        }

        var emailReg = /^([a-zA-Z-z0-9_\.-]+)@([\da-zA-Z\.-]+)\.([a-zA-Z\.]{2,6})$/;
        if (!emailReg.test(acct)) {
            this.showToast2("您输入的邮箱有误,请重新核对后再输入!");
            return false;
        }
    ...
    checkPhone: function (id) {
        ...
        if (!phone) {
            this.showToast2("电话不能为空");
            return false;
        }
        var phoneReg = /^(?:13\d|14\d|15\d|17\d|18\d)\d{5}(\d{3}|\*{3})$/;
        if (!phoneReg.test(phone)) {
            this.showToast2("您输入的电话号码有误,请重新核对后再输入!");
            return false;
        }
        return true;
    }
